...
If running inside a Ralph loop (prompt contains completion promise):
1. Do NOT output the promise until feature ACTUALLY WORKS
2. Take Playwright screenshot to verify UI
3. Trace the complete integration path (entry point → code → UI)
...
**Escape hatch:** If stuck after 10+ iterations, output `<promise>STUCK_NEED_HELP</promise>` to exit and escalate.
